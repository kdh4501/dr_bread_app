...
class GetRecipesUseCase {
  ...
  Stream<List<RecipeEntity>> call({RecipeFilterOptions? filterOptions}) { // call() 메서드 사용
    // 비즈니스 로직 수행 (예: 특정 조건에 맞는 레시피만 필터링 등 - 현재는 Repository 위임)

    // Repository에게 레시피 목록 조회 요청 위임 (실시간 스트림)
    return repository.getRecipes(filterOptions: filterOptions); // Repository의 getRecipes() 메서드 호출 (Stream 반환)
  }
